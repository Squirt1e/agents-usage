...
| | |
| --- | --- |
| ![连接状态](assets/screenshots/connection-status.png) | ![设置](assets/screenshots/settings.png) |
| **连接状态**：底部按连接列出失败原因，卡片布局不受影响。 | **设置**：平台显隐与排序、主题、额度数值。 |
| ![GLM 配置](assets/screenshots/glm-settings.png) | ![DeepSeek 配置](assets/screenshots/deepseek-settings.png) |
| **GLM 配置**：服务区域、Coding Plan 凭据、实验钱包连接。 | **DeepSeek 配置**：余额凭据与实验网页用量连接<br>（截图中凭据掩码已打码）。 |
...
- **实验连接**（GLM 钱包、DeepSeek 网页用量）：默认关闭，需要自己确认端点可用性再打开。
  ...
  “删除”按钮。它们使用非公开稳定接口，可能随时失效，故障不会影响同平台的稳定连接。
...
```
src-tauri/             Tauri 宿主：菜单栏、面板窗口生命周期、受限命令/事件桥
src/desktop/           面板前端（React + TS，约 350 逻辑像素的紧凑界面）
