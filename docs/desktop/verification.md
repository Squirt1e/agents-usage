...
- TypeScript：`tests/panel/contracts-conformance.test.ts`；
...
1. `tests/shared/contracts.test.ts`：`normalizeMetric` 对 `undefined` 产出 `null` 并追加 `unavailable`。
2. `tests/panel/contracts-conformance.test.ts`：`codex-windows.json` 要求缺失百分比的不兼容响应
...
1. `tests/panel/panel-motion.test.ts`：例外表登记这次逐帧绘制的原因与它仍然生效的条件，并覆盖它旁边
   真正是 CSS 的切换（填充比例、重置时间行的语气与下划线）。
2. `tests/panel/panel-quota-morph.test.tsx`：几何（两种静息形态、三段顺序、接缝处两端速度连续、每帧
   不出盒、无 NaN）与 DOM 行为（翻转提交当帧先写回真实帧、减弱动效直接落地、两种文字只在形变期间
   同时挂载、形变期间在 `.panel` 上标注"内容正在动画"）。
3. `tests/panel/panel-height-hook.test.tsx`：形变期间窗口高度改为逐帧上报实测值（不再等布局稳定后
...
5. `tests/panel/panel-width.test.ts`：面板不提供横向滚动（页面级 `clip`、内容区不让出横向轴、底部详情中的长文案允许断行）。浏览器侧的实测方式：向连接错误注入一段很长且不可断行的英文文案，打开底部“连接异常”详情，再读 `documentElement.scrollWidth`／`.panel-body.scrollWidth`，两者都必须等于各自的 `clientWidth`，且手动设置 `scrollLeft` 不产生位移。
...
7. `tests/panel/panel-quota-morph.test.tsx` 另有一条分层断言：切换按钮盖住整个条目且置于内容之下，
...
1. `tests/settings/settings-store.test.ts`：写入立刻采纳自己的答案、宿主广播同一个值算回声（只通知一次）、
   嵌套字段（`platformVisibility`）的变化能被认出来、`dispose()` 后不再监听。
2. `tests/settings/settings-window.test.tsx`：分类只挂载一个、方向键在同一控件内移动、`panel://settings-section`
   到达时切分类、`settings_ready` 只在首次渲染后发一次；两个窗口同屏时（`renderBothWindows`）
   改一项，面板上的卡片当场变化。
3. `tests/settings/panel-settings.test.tsx`：面板四个入口各自请求哪个 section；面板本身不再渲染设置界面。
...
5. `tests/panel/panel-motion.test.ts`：守卫读 `panel.css` 与 `settings.css` 两份样式，分类选中与内容入场
   都在登记表里；面板的页面转场条目已随页面一起删除。
6. `tests/panel/panel-width.test.ts`：设置窗口把面板的 350px 宽度与 320px 最小高度中和掉，且 `panel.css`
   自己不被这些中和改动。
...
1. **设置窗口的外观与尺寸**：从面板顶部齿轮打开，确认带系统标题栏、固定 600×400、不可拖动边缘
   缩放，内容超出时只有内容区滚动。
2. **位置**：设置窗口在当前显示器工作区水平居中，垂直中心比工作区中心上移 50 逻辑像素；紧凑工作
   区内仍被钳制在边缘内。
...
8. **面板高度**：隐藏全部平台后窗口不再停在 320，而是收到空状态需要的高度；卡片很多时窗口停在
   工作区高度并在内部滚动；钉住后面板失焦、标题栏收起时底部状态区同步隐藏，窗口跟着变矮且底部
   不留空白。

## 实机验证记录（设置窗口，2026-09-15，尺寸调整前基线）
